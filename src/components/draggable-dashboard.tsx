...
export const DraggableDashboard: React.FC<DraggableDashboardProps> = ({
  ...
  // NEW: Refresh key for leads table
  const [leadsRefreshKey, setLeadsRefreshKey] = useState<number>(0);

  // Callback to trigger leads table refresh after upload
  const handleLeadsUploadComplete = useCallback(() => {
  ...
  const allDashboardItems: DashboardItem[] = [
    {
      ...
      component: (
        <LeadsTable
          initialLeads={initialData.leads}
          initialMarketRegions={initialData.marketRegions}
          userRole={userRole}
          userId={userId}
    ...
    },
    {
      ...
      component: <LeadsUpload onUploadComplete={handleLeadsUploadComplete} />,
    },
    {
      i: "senders",
      title: "Email Senders",
